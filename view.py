...
import sys 
from utils import AuthorizationService,UserFeatureLabelResolever,UserFeatureContentPanelResolver
...
class TwoPanelLayoutApp(MDApp):
    ...
    def _create_navigation_bar_components(self):
        ...
        self.user_features =authorization_service.get_feature_user_by_user_role(user_role)
        
        for feature in self.user_features:
            button  = Button(text = UserFeatureLabelResolever.get_user_feature_label(feature),background_color =(0,1,1,1),
                            color = (1,1,1,1),
                            font_size = 18,
                            size_hint = (1,None))
            button.size = (300,60)
